...
$available_font_families = array(
    "'Open Sans'" => "Open Sans",
);
foreach ($font_manager->get_all_fonts_name() as $font_name) {
    $available_font_families["'" . $font_name . "'"] = $font_name;
}

$available_font_weights = array(
    "normal" => "Normal",
    "bold" => "Bold",
    "bolder" => "Bolder",
    "lighter" => "Lighter",
...
$theme_options = new Finesse_Theme_Options();

class Finesse_Theme_Options
{
    ...
    {
        $xml = simplexml_load_file(TEMPLATEPATH . "/api/theme-options.xml");
        foreach ($xml->section as $section) {
            array_push($this->sections, new Finesse_Section($section));
        }
    ...
    {
        $theme_options_title = __('Theme Options', 'finesse');
        add_theme_page($theme_options_title, $theme_options_title, 'manage_options',
            'finesse-theme-options', array($this, 'render'));
    }
    ...
    function register_options()
    {
        ...
        foreach ($this->sections as $section) {
            foreach ($section->categories as $category) {
                foreach ($category->options as $option) {
                    register_setting('finesse-options', $option->name);
                }
            }
        }
    }
    ...
    function render()
    {
        ...
        echo "<h2>Finesse - Theme Options</h2>\n";
        ...
        settings_fields('finesse-options');
        ...
        echo "<p>";
        echo '<input type="submit" class="button-primary" value="' . __('Save Changes', 'finesse') . '"/>';
        echo '<input id="restore-theme-settings" type="submit" class="button-secondary" value="' . __('Restore Defaults', 'finesse') . '"/>';
        echo "</p>";
...
class Finesse_Section
{
    ...
    function __construct($element)
    {
        ...
        if (isset($element->category)) {
            ...
            for ($i = 0; $i < $cnt; $i++) {
                array_push($this->categories, new Finesse_Category($element->category[$i]));
            }
        }
    }
    ...
    function render_title($is_selected)
    {
        ...
        echo '<a class="nav-tab' . $selected_tab_class . '" href="themes.php?page=finesse-theme-options&tab=' . $this->id . '">' . $this->title . '</a>';
    }
...
class Finesse_Category
{
    ...
    function __construct($element)
    {
        ...
        if (isset($element->option)) {
            ...
            for ($i = 0; $i < $cnt; $i++) {
                array_push($this->options, new Finesse_Option($element->option[$i]));
            }
        }
    }
